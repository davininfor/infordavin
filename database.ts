...
export function initDb() {
  ...
  if (profileCount.count === 0) {
    db.prepare(`
      INSERT INTO profile (name, email, instagram, whatsapp, github, tagline, about)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(
    ...
    const projects = [
      ['Personal Portfolio', 'A premium portfolio website built with React and SQLite.', 'https://picsum.photos/seed/portfolio/800/600', 'https://github.com/davininfor'],
      ['Esports Dashboard', 'A UI design concept for tracking tournament stats.', 'https://picsum.photos/seed/esports/800/600', '#'],
      ['Task Manager App', 'A simple CRUD application for managing daily tasks.', 'https://picsum.photos/seed/tasks/800/600', '#']
    ];
    const insertProject = db.prepare('INSERT INTO projects (title, description, image, link) VALUES (?, ?, ?, ?)');
